...
export function QuestionDialog({ open, onOpenChange, question }: QuestionDialogProps) {
  ...
  const isLoading = createMutation.isPending || updateMutation.isPending || categoriesLoading || levelsLoading
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      const payload = {
        ...
        levelId: value.levelId === 'none' ? null : value.levelId,
        questionType: value.questionType,
        difficulty: value.difficulty,
        scenarioEn: value.scenarioEn.trim() || null,
  ...
  useEffect(() => {
    if (open) {
      if (question) {
        form.setFieldValue('categoryId', question.category_id)
        form.setFieldValue('levelId', question.level_id || 'none')
        ...
        form.setFieldValue('scenarioEn', question.scenario_en || '')
        form.setFieldValue('scenarioKh', question.scenario_kh || '')
  ...
  // Get reactive category value from form field state
  const selectedCategoryId = useStore(form.store, (s) => s.values.categoryId)
  const currentQuestionType = useStore(form.store, (s) => s.values.questionType)

  // Filter levels client-side relative to CategoryId selection
  const filteredLevels = allLevels.filter((l) => l.category_id === selectedCategoryId)
      ...
      <DialogContent className="sm:max-w-[960px] md:max-w-[960px] w-[95vw] text-slate-900 dark:text-slate-50 border border-slate-100 dark:border-slate-800 shadow-xl max-h-[90vh] !flex !flex-col !p-0 !gap-0 overflow-hidden">
        <DialogHeader className="p-6 pb-2 space-y-1.5 shrink-0">
          ...
          <DialogDescription className="text-xs text-slate-500 dark:text-slate-400">
            {isEditing
              ? 'Update translations, parameters, levels, or formatting options.'
              : 'Add a new educational gameplay question with answers validation.'}
          </DialogDescription>
        </DialogHeader>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Select
                        ...
                        onValueChange={(val) => {
                          ...
                          form.setFieldValue('levelId', 'none') // Reset level
                        }}
                  ...
                  children={(field) => (
                    <div className="space-y-1.5">
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="none">No Level Linked</SelectItem>
