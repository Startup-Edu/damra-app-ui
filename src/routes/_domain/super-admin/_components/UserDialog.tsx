...
export function UserDialog({ open, onOpenChange, user }: UserDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      if (user) {
        ...
        setName(user.name)
        setPassword('')
        // user.roles is an array, we assign the first one's ID
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Limit to 2MB to prevent payload issues
          ...
          <div className="flex flex-col gap-3.5 pt-1.5">
            ...
            {isEditing && (
              <div className="flex items-start gap-2.5 p-3 rounded-lg border border-amber-500/10 bg-amber-500/5">
                ...
                />
                <div className="space-y-0.5">
                  <Label htmlFor="reset-pass" className="text-[11px] font-semibold text-amber-700 dark:text-amber-400 cursor-pointer flex items-center gap-1.5">
                  ...
                  <p className="text-[9px] text-amber-600/70 dark:text-amber-400/60 leading-tight">
                    The user will be required to configure a new password during their next login sequence.
                  </p>
                </div>
              </div>
            )}
